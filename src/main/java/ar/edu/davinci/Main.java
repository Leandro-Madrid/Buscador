...
public class Main {
    public static void main(String[] args) {
        ...
        List<PaginaWeb> resultadosPorAutor = buscador.buscar(listaPaginas, pagina ->
                pagina.getArticulos().stream()
                        .anyMatch(articulo -> articulo.getAutor().getNombre().toLowerCase().contains("Juan")));
        System.out.println("Resultados por autor 'Juan': " + resultadosPorAutor.size());


        List<PaginaWeb> resultadosPorTema = buscador.buscar(listaPaginas, pagina ->
                pagina.getTemas().stream()
                        .anyMatch(tema -> tema.toLowerCase().contains("Tema2")));
        System.out.println("Resultados por tema 'Tema2': " + resultadosPorTema.size());


        List<PaginaWeb> resultadosPorFecha = buscador.buscar(listaPaginas, pagina ->
                pagina.getArticulos().stream()
                        .anyMatch(articulo -> articulo.getFechaPublicacion().equals("02/10/2024")));
        System.out.println("Resultados por fecha '02/10/2024': " + resultadosPorFecha.size());


        List<PaginaWeb> resultadosPorUrl = buscador.buscar(listaPaginas, pagina ->
                pagina.getUrl().toLowerCase().contains("pagina1"));
        System.out.println("Resultados por URL que contienen 'pagina1': " + resultadosPorUrl.size());
    }
}